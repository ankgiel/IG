...
class ObjJerarquico
{
   public:
      ...
      void draw( const int modo_draw, const bool ajedrez, const int material );

		void decrementaParam(unsigned n_param);
		
		
		void incrementaParam(unsigned n_param);
		void aceleraParam(unsigned n_param);
		void deceleraParam(unsigned n_param);
   ...
   private:
      ...
      std::vector<float> valores_na ;
		std::vector<float> delta_valores ;
